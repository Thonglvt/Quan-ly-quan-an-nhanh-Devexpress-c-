...
namespace GUI
{
    public partial class FrmCaLamViec : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void loadSearchLookupEdit()
        {
            ...
            sleNhanVien.Properties.DataSource = bllNV.getAll();
        ...
        private void btnDangKyCa_Click(object sender, EventArgs e)
        {
            ...
            var maCa = gvCaLamViec.GetRowCellValue(gvCaLamViec.FocusedRowHandle, gvCaLamViec.Columns["MaCa"]);
            var tenCa = gvCaLamViec.GetRowCellValue(gvCaLamViec.FocusedRowHandle, gvCaLamViec.Columns["TenCa"]);
            if(string.IsNullOrEmpty(sleNhanVien.EditValue.ToString())|| sleNhanVien.EditValue.ToString().Equals("Chọn nhân viên")||sleNhanVien.EditValue.ToString().Equals("[EditValue ifs null]"))
            {
                XtraMessageBox.Show("Vui lòng chọn nhân viên cần đăng ký ca", "Thông báo [Message]"
                         , MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void sleNhanVien_BeforePopup(object sender, EventArgs e)
        {
            loadSearchLookupEdit();
        }
    }
